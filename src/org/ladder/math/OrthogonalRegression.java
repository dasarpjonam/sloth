...
import org.ladder.math.linear.Vector2D;
...
public class OrthogonalRegression {
	 ...
	 */
	public static Line2D getLineFit(List<IPoint> points){
		double xBar = 0;
		double yBar = 0;
		for(IPoint p : points){
			xBar+=p.getX();
		...
		for(int i=1; i<S.getColumnDimension();i++)
			if(S.get(i, i)<S.get(index, index))
				index=i;
		double lineNormalDirection = Math.atan2(V.get(1, index), V.get(0, index));
		...
		return new Line2D.Double(xBar,yBar,xBar+Math.cos(lineDirection),yBar+Math.sin(lineDirection));
	}
	...
	public static void main(String[] args){
		...
		for(int i=0;i<10;i++){
			...
			points.add(new Point(i+(rand.nextDouble()-.5)/100.0,i+(rand.nextDouble()-.5)/100.0));
		}

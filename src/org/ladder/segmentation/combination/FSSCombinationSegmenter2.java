...
import org.ladder.segmentation.combination.objectiveFunctions.IObjectiveFunction;
import org.ladder.segmentation.combination.objectiveFunctions.PolylineMSEObjectiveFunction;
 ...
 * @author awolin
 */
public class FSSCombinationSegmenter2 extends AbstractSegmenter implements
        ISegmenter {
	 ...
	 * Segmenter name
	 */
	private static final String S_SEGMENTER_NAME = "FSS Combination Segmenter 2";
	
	/**
	 * SBFS threshold
	 */
	private static final double S_THRESHOLD = 1.9887473405191085; //1.7637226;//2.020747995;
	...
	private List<ISegmentation> m_shortStrawSegs;
	
	private List<ISegmentation> m_dpSegs;
	
	private List<ISegmentation> m_sezginSegs;
	
	private List<ISegmentation> m_paleoSegs;
	
	private List<ISegmentation> m_kimSegs;
	
	
	/**
	 * Default constructor
	 */
	public FSSCombinationSegmenter2() {
		this(false);
	}
	 ...
	 */
	public FSSCombinationSegmenter2(boolean useSmoothing) {
	 ...
	 *            of not (needed for PaleoSegmenter)
	 */
	public FSSCombinationSegmenter2(SubsetSelectionAlgorithm fssType,
	 ...
	 *            of not (needed for PaleoSegmenter)
	 */
	public FSSCombinationSegmenter2(IObjectiveFunction objFunction,
	        boolean useSmoothing) {
		this(useSmoothing);
	...
	public List<ISegmentation> getShortStrawSegs() {
		return m_shortStrawSegs;
	}
	

	public List<ISegmentation> getDPSegs() {
		return m_dpSegs;
	}
	

	public List<ISegmentation> getSezginSegs() {
		return m_sezginSegs;
	}
	

	public List<ISegmentation> getPaleoSegs() {
		return m_paleoSegs;
	}
	

	public List<ISegmentation> getKimSegs() {
		return m_kimSegs;
	}
	...
	public List<ISegmentation> getSegmentations()
		...
		// uber hack - bypass if we have too many corners
		if (allCorners.size() > 100)
			return new ArrayList<ISegmentation>();
		...
		List<ISegmentation> combinedSegmentations = segmentStroke(m_stroke,
	...
	private List<Integer> getInitialCorners() throws InvalidParametersException {
		...
		m_shortStrawSegs = segmenter.getSegmentations();
		allCorners.addAll(getCornersFromSegmentations(m_shortStrawSegs));
		
		// Douglas-Peucker
		segmenter = new DouglasPeuckerSegmenter(m_stroke);
		m_dpSegs = segmenter.getSegmentations();
		allCorners.addAll(getCornersFromSegmentations(m_dpSegs));
		
		// Paleo
		segmenter = new PaleoSegmenter(m_useSmoothing);
		segmenter.setStroke(m_stroke);
		m_paleoSegs = segmenter.getSegmentations();
		allCorners.addAll(getCornersFromSegmentations(m_paleoSegs));
		
		// Sezgin
		segmenter = new SezginSegmenter();
		segmenter.setStroke(m_stroke);
		m_sezginSegs = segmenter.getSegmentations();
		allCorners.addAll(getCornersFromSegmentations(m_sezginSegs));
		
		// Kim
		segmenter = new KimSquaredSegmenter();
		segmenter.setStroke(m_stroke);
		m_kimSegs = segmenter.getSegmentations();
		allCorners.addAll(getCornersFromSegmentations(m_kimSegs));
	 ...
	 */
	@Deprecated
	private List<Integer> sfs(List<Integer> corners, IStroke stroke,
	...
	@Deprecated
	private List<Integer> sbs(List<Integer> corners, IStroke stroke,
	        IObjectiveFunction objFunction) {
	...
	@SuppressWarnings("unchecked")
	@Deprecated
	private List<Integer> sffs(List<Integer> corners, IStroke stroke,
	        IObjectiveFunction objFunction) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<Integer> sbfs(List<Integer> corners, IStroke stroke,
	        IObjectiveFunction objFunction) {
	...
	private List<Object> nextBestCorner(List<Integer> cornerSubset,
	        List<Integer> corners, IStroke stroke,
	        IObjectiveFunction objFunction) {
	...
	private List<Object> prevBestSubset(List<Integer> cornerSubset,
	        IStroke stroke, IObjectiveFunction objFunction) {
	 ...
	 * @see org.ladder.segmentation.ISegmenter#getSegmentations()
	 */
	public List<ISegmentation> getSegmentationsForCVTraining(int knownNumCorners)
	        throws InvalidParametersException {
	...
	private List<Integer> sbfsForCVTraining(List<Integer> corners,
	        IStroke stroke, IObjectiveFunction objFunction, int knownNumCorners) {
	 ...
	 * @throws InvalidParametersException
	 */
	public List<ISegmentation> getSegmentationsForCVTesting(double threshold)
	        throws InvalidParametersException {
	...
	private List<Integer> sbfsForCVTesting(List<Integer> corners,
	        IStroke stroke, IObjectiveFunction objFunction, double threshold) {

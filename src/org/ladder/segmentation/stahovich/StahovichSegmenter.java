package org.ladder.segmentation.stahovich;
	...
	private List<Integer> stahovichFit(double[] curvatures, double[] speeds,
		...
		for (int i = 0; i < corners.size() - 1; i++) {
			...
			if (errRatioLeft < errRatioRight && (errRatioLeft - 1.0) < 0.1) {
				...
				corners.remove(i);
				i--;
			} else if ((errRatioRight - 1.0) < 0.1) { // && pathRatioRight < 0.2) {
				corners.remove(i + 1);
				i--;
			}
		}
...
//		double[] signedCurvatures = calcSignedCurvatures(pathLengths, directions, true);
//		
...
//			double err0 = arcOrthogonalDistanceSquared(m_stroke.getPoints(), c0, c1);
//
...
//					double errLeft = arcOrthogonalDistanceSquared(m_stroke.getPoints(), c0, cs);
//					double errRight = arcOrthogonalDistanceSquared(m_stroke.getPoints(), cs, c1);
//					
...
//		}
	 ...
	 */
	@Override
	protected double arcOrthogonalDistanceSquared(List<IPoint> pts, int p1,
			int p2) {

		double error = 0.0;

		IPoint pt1 = pts.get(p1);
		IPoint pt2 = pts.get(p2);
		...
		IPoint midPoint = new Point(midX, midY);

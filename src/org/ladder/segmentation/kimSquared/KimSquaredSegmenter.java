package org.ladder.segmentation.kimSquared;
...
public class KimSquaredSegmenter extends AbstractSegmenter implements
	 ...
	 *            Stroke to segment
	 */
	public KimSquaredSegmenter(IStroke stroke) {
		setStroke(stroke);
	}
	...
	public void setStroke(IStroke stroke) {
		...
		List<IPoint> pts = resamplePoints(m_stroke, resampleSpacing);
		
		// Original Kim algorithm had 5.0 pixel resample spacing
		// List<IPoint> pts = resamplePoints(m_stroke,
	 ...
	 */
	private double[] calcArcLengths(IStroke stroke) {
	 ...
	 */
	private double[] calcKimDirections(IStroke stroke) {
		int numPts = stroke.getNumPoints();
	 ...
	 */
	private double direction(List<IPoint> pts, int index) {
		if (index - 1 >= 0) {

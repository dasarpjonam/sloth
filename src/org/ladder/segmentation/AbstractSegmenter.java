package org.ladder.segmentation;
...
public abstract class AbstractSegmenter {
	 ...
	 */
	protected Stroke cleanStroke(IStroke uncleanedStroke) {
		
		List<IPoint> pts = new ArrayList<IPoint>(uncleanedStroke.getPoints());
	 ...
	 * {@link #isLine(IStroke, int, int, double[])} test.
	 * 
	 * @param stroke
	 ...
	 *         that array's index
	 */
	protected double[] calcPathLengths(IStroke stroke) {
	 ...
	 */
	protected boolean isLine(int p1, int p2, IStroke stroke,
		...
		IPoint pt1 = stroke.getPoint(p1);
		IPoint pt2 = stroke.getPoint(p2);
	 ...
	 */
	protected List<ISegmentation> segmentStroke(IStroke stroke,
		...
		segmentation.setConfidence(confidence);
	...
	protected List<Integer> getOriginalStrokeCorners(
	        List<Integer> alteredCorners, IStroke alteredStroke,
	        IStroke origStroke) {
	...
	protected List<Integer> getCornersFromSegmentations(
		...
		for (ISegmentation seg : segmentations) {
	...
	private int findClosestOriginalCorner(int alteredIndex,
	        IStroke alteredStroke, IStroke origStroke) {
	 ...
	 *         given time
	 */
	private int closestTimeSearch(List<IPoint> points, long time, int low,
	        int high) {

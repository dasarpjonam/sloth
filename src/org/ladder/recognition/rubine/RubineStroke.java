...
public class RubineStroke extends Stroke {
	 ...
	 */
	public RubineStroke(IStroke stroke) {
	 ...
	 *            Set of features
	 */
	public RubineStroke(IStroke stroke, FeatureSet featureSet) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.ladder.patternrec.classifiers.IClassifiable#getClassLabel()
	 */
	...
	private void calcFeatures() {
		...
		List<IPoint> featurePoints = cleanStroke(this);
		...
		switch (m_featureSet) {
			...
			case Long:
				...
				break;
			case COA:
				addCOAFeatures(featurePoints, distDeltas, m_angleDeltas,
				        timeDeltas);
				break;
			default:
				addRubineFeatures(featurePoints, distDeltas, m_angleDeltas,
	 ...
	 */
	private List<IPoint> cleanStroke(IStroke stroke) {
		
		// Create a copied version of the points
		List<IPoint> cleanedPoints = new ArrayList<IPoint>();
		
		for (IPoint p : stroke.getPoints()) {
			cleanedPoints.add((IPoint) p.clone());
		}
	 ...
	 */
	private List<IPoint> standardizeStrokePoints(List<IPoint> origPoints) {
		
		// Create a copied version of the points
		List<IPoint> scaledPoints = new ArrayList<IPoint>();
		List<IPoint> transposedPoints = new ArrayList<IPoint>();
		
		for (IPoint p : origPoints) {
			scaledPoints.add((IPoint) p.clone());
		}
		...
		for (int i = 0; i < scaledPoints.size(); i++) {
			IPoint point = scaledPoints.get(i);
		...
		for (int i = 0; i < transposedPoints.size(); i++) {
			IPoint point = transposedPoints.get(i);
	 ...
	 * @return Stroke standardized to a certain bounding box width and height
	 */
	private List<IPoint> transposeStrokePoints(List<IPoint> origPoints) {
	 ...
	 * @return
	 */
	private List<IPoint> swapYAxis(List<IPoint> points, double maxY) {
		
		List<IPoint> swappedPoints = new ArrayList<IPoint>();
	 ...
	 */
	private Point2D.Double distChangeAtPoint(int p, List<IPoint> featurePoints) {
		
		IPoint point = featurePoints.get(p);
		IPoint nextPoint = featurePoints.get(p + 1);
	 ...
	 */
	private double angleChangeAtPoint(int p, List<IPoint> featurePoints,
	 ...
	 */
	private long timeChangeAtPoint(int p, List<IPoint> featurePoints) {
	 ...
	 */
	private void addRubineFeatures(List<IPoint> featurePoints,
	 ...
	 *            Stroke points to calculate features for
	 */
	private void addLongFeatures(List<IPoint> featurePoints,
	        Point2D.Double[] distDeltas, double[] angleDeltas, long[] timeDeltas) {
	 ...
	 *            Stroke points to calculate features for
	 */
	private void addCOAFeatures(List<IPoint> featurePoints,
	        Point2D.Double[] distDeltas, double[] angleDeltas, long[] timeDeltas) {
	 ...
	 */
	private BoundingBox getBoundingBox(List<IPoint> featurePoints) {
	 ...
	 */
	private double initialCosine(List<IPoint> featurePoints) {
		
		IPoint p0 = featurePoints.get(0);
	 ...
	 */
	private double initialSine(List<IPoint> featurePoints) {
	 ...
	 */
	private double firstLastPtDist(List<IPoint> featurePoints) {
		IPoint firstPoint = featurePoints.get(0);
		IPoint lastPoint = featurePoints.get(featurePoints.size() - 1);
	 ...
	 */
	private double firstLastCosine(List<IPoint> featurePoints,
	 ...
	 */
	private double firstLastSine(List<IPoint> featurePoints,
	        double firstLastPtDist) {
	 ...
	 */
	private double totalLength(List<IPoint> featurePoints,
	        Point2D.Double[] distDeltas) {
		...
		for (int p = 0; p < featurePoints.size() - 1; p++) {
			pathLength += Math.sqrt((distDeltas[p].getX() * distDeltas[p]
			        .getX())
			                        + (distDeltas[p].getY() * distDeltas[p]
			                                .getY()));
		}
	 ...
	 */
	private double totalAngle(List<IPoint> featurePoints, double[] angleDeltas) {
	 ...
	 */
	private double totalAbsAngle(List<IPoint> featurePoints,
	        double[] angleDeltas) {
	 ...
	 */
	private double smoothness(List<IPoint> featurePoints, double[] angleDeltas) {
	 ...
	 */
	private double maxSpeed(List<IPoint> featurePoints,
	 ...
	 */
	private double totalTime(List<IPoint> featurePoints) {
		
		IPoint firstPoint = featurePoints.get(0);
		IPoint lastPoint = featurePoints.get(featurePoints.size() - 1);
		
		return (double) (lastPoint.getTime() - firstPoint.getTime());
	}
